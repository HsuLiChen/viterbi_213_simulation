...
% txBits = load('msg_data_8191', '-ascii');
...
for ii = 1:length(EsN0_dB)
    ...
    for k_tb = 2:2:16
        for num = 1:8
            decoded_matlab = vitdec(rxBits_hard,trellis,k_tb, 'trunc' , 'hard' );
            % 位元錯誤率 (BER) 計算
            [~, ber_software_tracback] = biterr(txBits, decoded_matlab);
            BER_software_tracback_result(num,ii) = ber_software_tracback;
        end
    end
...
[k_max, num_EsN0] = size(BER_software_tracback_result);
markers = {'-*', '-d', '-^', '-v', '-<', '->', '-p', '-h'}; % 8種不同標記，對應 traceback 長度
for k_tb_idx = 1:k_max
    semilogy(EsN0_dB, BER_software_tracback_result(k_tb_idx, :), ...
        markers{mod(k_tb_idx-1, length(markers))+1}, 'LineWidth', 1.5, ...
        'DisplayName', sprintf('Software Viterbi TB=%d', k_tb_idx*2));
end
