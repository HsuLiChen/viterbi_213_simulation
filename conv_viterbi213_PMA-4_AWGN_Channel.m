...
for ii = 1:length(EsN0_dB)
    ...
    BER_hard(ii) = ber_hard;
     ...
     % 位元錯誤率 (BER) 計算
    [~, ber_matlab] = biterr(txBits, decoded_matlab);
    BER_matlab(ii) = ber_matlab;

    % 位元錯誤率 (BER) 計算
    [~, ber_HARDV3] = biterr(txBits, rxBits_hardV3);
    BER_HARDV3(ii) = ber_HARDV3;
end
...
for ii = 1:length(EsN0_dB)
    ...
    theorySER(ii) = 3/2 * qfunc( sqrt(0.8 * gamma) ); 
end


%% 繪圖 (BER / SER)
figure;
semilogy(EsN0_dB, BER, 'ko-', 'LineWidth',1.5); hold on;
semilogy(EsN0_dB, BER_hard, 'go-', 'LineWidth',1.5);
semilogy(EsN0_dB, BER_matlab, 'yo-', 'LineWidth',1.5);
semilogy(EsN0_dB, BER_HARDV3, 'ro-', 'LineWidth',1.5);


grid on;
title('(2,1,3) in 4-PAM_AWGN Channel BER/SER with Different Traceback Depths');
xlabel('E_s/N_0 (dB)');
ylabel('Bit Error Rate (BER)');
legend('Uncoded System', 'Hardware Coded System', 'MATLAB Coded System','Hardware Coded (tb =12) System', 'Location', 'best');
legend show;
hold off;
